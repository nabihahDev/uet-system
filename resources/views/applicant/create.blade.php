    ...
    <div class="py-6 bg-slate-200 min-h-screen text-slate-900 font-sans text-xs">
        <div class="max-w-7xl mx-auto bg-white p-6 md:p-8 shadow-md border border-slate-400">
            ...
            <form action="{{ route('uet.store') }}" method="POST" enctype="multipart/form-data" id="uetForm">
                    ...
                    <table class="w-full border-collapse border border-slate-900 text-[10px]">
                        <thead>
                            ...
                            <tr class="bg-slate-100 text-center font-bold divide-x divide-slate-900 border-b border-slate-900">
                                <th colspan="11" class="py-1 uppercase bg-slate-200">DIISI OLEH PEMOHON DENGAN LENGKAP</th>
                                <th colspan="2" class="py-1 uppercase bg-slate-300">DIISI OLEH JAWATANKUASA & YANG BERKENAAN SAHAJA</th>
                            </tr>
                            <!-- Column Titles -->
                            <tr class="bg-slate-50 text-center font-bold divide-x divide-slate-900 border-b border-slate-900">
                                ...
                                <th class="p-1 w-32 bg-slate-100">KEPUTUSAN JKG / ECC / MCSC / UETSC</th>
                        ...
                        <tbody id="itemTableBody" class="divide-y divide-slate-900">
                            ...
                            <tr class="divide-x divide-slate-900 relative group">
                                ...
                                <!-- (a) - (i): Pemohon Fields -->
                                <td class="p-1">
                                    <input type="text" name="items[0][sub_unit]" 
                                           @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') readonly @endif
                                ...
                                <td class="p-1">
                                    <input type="number" name="items[0][qty_dipohon]" 
                                           @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') readonly @endif
                                ...
                                <td class="p-1 text-center">
                                    <input type="checkbox" name="items[0][dalam_simpanan_ada]" value="1" 
                                           @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') disabled @endif>
                                </td>
                                <td class="p-1 text-center">
                                    <input type="checkbox" name="items[0][dalam_simpanan_tiada]" value="1" 
                                           @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') disabled @endif>
                                </td>
                                <td class="p-1">
                                    <input type="text" name="items[0][muka_surat_jku]" 
                                           @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') readonly @endif
                                ...
                                <td class="p-1">
                                    <select name="items[0][pindaan_type]" 
                                            @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') disabled @endif
                                ...
                                <td class="p-1">
                                    <textarea name="items[0][alasan]" rows="2" 
                                              @if(auth()->user()->role === 'oc' || auth()->user()->role === 'qm') readonly @endif
                                              class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none {{ auth()->user()->role === 'oc' || auth()->user()->role === 'qm' ? 'bg-slate-100 cursor-not-allowed' : '' }}"></textarea>
                                </td>

                                <!-- (j) Ulasan Timb Peg Turus: EDITABLE ONLY BY OC -->
                                <td class="p-1 bg-amber-50/50">
                                    <textarea name="items[0][ulasan_timb_peg_turus]" rows="2" 
                                              @if(auth()->user()->role !== 'oc') readonly @endif 
                                              placeholder="{{ auth()->user()->role === 'oc' ? 'Masukkan ulasan...' : '' }}"
                                              class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none 
                                                     {{ auth()->user()->role !== 'oc' ? 'bg-transparent cursor-not-allowed text-slate-500' : 'bg-white border border-amber-300' }}"></textarea>
                                </td>

                                <!-- (k) Keputusan JKG/ECC: EDITABLE ONLY BY QM / COMMITTEE -->
                                <td class="p-1 bg-slate-50">
                                    <textarea name="items[0][keputusan_jkg]" rows="2" 
                                              @if(auth()->user()->role !== 'qm') readonly @endif 
                                              class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none 
                                                     {{ auth()->user()->role !== 'qm' ? 'bg-transparent cursor-not-allowed text-slate-500' : 'bg-white border border-slate-300' }}"></textarea>
                                </td>

                                <!-- (l) Catatan & Tindakan JKU: EDITABLE ONLY BY QM / COMMITTEE -->
                                <td class="p-1 bg-slate-50 relative">
                                    <textarea name="items[0][catatan_jku]" rows="2" 
                                              @if(auth()->user()->role !== 'qm') readonly @endif 
                                              class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none 
                                                     {{ auth()->user()->role !== 'qm' ? 'bg-transparent cursor-not-allowed text-slate-500' : 'bg-white border border-slate-300' }}"></textarea>
                                    
                                    @if(auth()->user()->role !== 'oc' && auth()->user()->role !== 'qm')
                                        <button type="button" class="remove-row text-red-600 font-bold absolute right-1 top-1 hidden">×</button>
                                    @endif
                                </td>
                            </tr>
                        </tbody>
                ...
                <!-- Add Row Control (Only for Pemohon) -->
                @if(auth()->user()->role !== 'oc' && auth()->user()->role !== 'qm')
                ...
                <!-- BOTTOM SECTIONS: Signatures & Official Sections (2 to 8 Side-by-Side) -->
                <div class="grid grid-cols-12 gap-6 border-t-2 border-slate-900 pt-4">

                    <!-- LEFT SIDE: Section 2, 3, 4, 5 (Applicant & Committee Side) -->
                    <div class="col-span-12 md:col-span-7 space-y-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex-1 border border-dashed border-slate-900 h-8 px-2 flex items-center bg-white">
                                <input type="text" 
                                    ...
                                    @if(auth()->user()->role !== 'oc') readonly @endif
                                    class="w-full border-0 p-0 text-xs focus:ring-0 
                                            {{ auth()->user()->role !== 'oc' ? 'cursor-not-allowed text-slate-500' : 'bg-transparent' }}">
                            </div>
                        </div>

                        <!-- 4. Keputusan Jawatankuasa (OC ONLY) -->
                            ...
                            <div class="pl-4 space-y-2">
                                <label class="flex items-center gap-2 {{ auth()->user()->role !== 'oc' ? 'pointer-events-none' : '' }}">
                                    ...
                                    <input type="text" 
                                        ...
                                        @if(auth()->user()->role !== 'oc') readonly @endif 
                                        class="border border-slate-800 px-2 py-0.5 text-xs w-28 focus:ring-0 
                        ...
                        <!-- 5. Setiausaha ECC/MCSC/UETSC (OC ONLY) -->
                    ...
                    <!-- RIGHT SIDE: Section 6, 7, 8 (Quartermaster / QM ONLY) -->
                    <div class="col-span-12 md:col-span-5 border border-slate-900 p-3 bg-slate-50 space-y-4 flex flex-col justify-between">
                        <div>
                            ...
                            <!-- 6. Pembantu Staf JKU -->
                            <div class="mb-3">
                                <label class="block font-bold mb-1 text-[11px]">
                                ...
                                <input type="text" 
                                       ...
                                       @if(auth()->user()->role !== 'qm') readonly @endif 
                                       class="w-full border border-slate-400 p-1.5 text-xs h-8
    ...
    <script>
        ...
        if (addBtn) {
            addBtn.addEventListener('click', function() {
                ...
                const isOc = (userRole === 'oc');
                const isQm = (userRole === 'qm');
                ...
                newRow.innerHTML = `
                    ...
                    <td class="p-1">
                        <textarea name="items[${rowIdx}][alasan]" rows="2" ${!isPemohon ? 'readonly' : ''} class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none ${!isPemohon ? 'bg-slate-100 cursor-not-allowed' : ''}"></textarea>
                    </td>
                    <td class="p-1 bg-amber-50/50">
                        <textarea name="items[${rowIdx}][ulasan_timb_peg_turus]" rows="2" ${!isOc ? 'readonly' : ''} class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none ${!isOc ? 'bg-transparent cursor-not-allowed text-slate-500' : 'bg-white border border-amber-300'}"></textarea>
                    </td>
                    <td class="p-1 bg-slate-50">
                        <textarea name="items[${rowIdx}][keputusan_jkg]" rows="2" ${!isQm ? 'readonly' : ''} class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none ${!isQm ? 'bg-transparent cursor-not-allowed text-slate-500' : 'bg-white border border-slate-300'}"></textarea>
                    </td>
                    <td class="p-1 bg-slate-50 relative">
                        <textarea name="items[${rowIdx}][catatan_jku]" rows="2" ${!isQm ? 'readonly' : ''} class="w-full text-[10px] p-0.5 border-0 focus:ring-0 resize-none ${!isQm ? 'bg-transparent cursor-not-allowed text-slate-500' : 'bg-white border border-slate-300'}"></textarea>
